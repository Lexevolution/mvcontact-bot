    ...
    signalRConnection.on("ReceiveMessage", (message) => {
        ...
        if (message.messageType == "Text"){
            if (message.content.startsWith("/echo ")){
                ...
                if (message.senderId === "U-Lexevo"){
                    const outUser = message.content.split(' ')[1];
                    const outMsg = message.content.split(' ').slice(2).join(' ');
                    ...
                    signalRConnection.send("SendMessage", {
                        ...
                        "sendTime": (new Date(Date.now())).toISOString(),
                        "lastUpdateTime": (new Date(Date.now())).toISOString(),
                        "content": `Sent message to ${outUser}!`
                    });
                }
            }
            else {
        ...
        if (message.messageType == "Sound"){
            const audioHash = JSON.parse(message.content).assetUri.slice(10,74);
            const audioURL = `https://assets.neos.com/assets/${audioHash}`;
            signalRConnection.send("SendMessage", {
                "id": `MSG-${randomUUID()}`,
                "senderId": loggedInData.userId,
                "recipientId": message.senderId,
                ...
                "sendTime": (new Date(Date.now())).toISOString(),
                "lastUpdateTime": (new Date(Date.now())).toISOString(),
                "content": "Listening to audio message..."
            });
